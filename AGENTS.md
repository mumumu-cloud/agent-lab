...
## 업무 보고 (Notion) — 커밋/푸시 시 필수

커밋 또는 푸시를 진행할 때마다, Notion "마케팅 업무 보고 (MY)" 페이지의
**4-4. 기타** 표에 업무 보고 내용을 요약 정리해 기재한다.

- 페이지: https://app.notion.com/p/MY-3d7a95abadc5803d8926fae966cc8728
- 형식(기존 4-4 기타 양식과 동일):
  - `이번주` 열에 `  • ` 불릿으로 작업 요약을 추가(줄바꿈은 `<br>`).
  - 바로 아래 행에 비개발자도 이해할 수 있는 쉬운 설명을 추가.
- **내가 작성하는 업무 보고 텍스트는 노란색(`<span color="yellow">…</span>`)으로 기재**한다.
- 기존에 강조 표기된 레드(빨간) 컬러 등 다른 사람이 작성한 서식은 변경하지 않는다.
- 기존에 기재된 내용은 삭제하지 않고, 새 작업을 이어서 추가·갱신한다.
